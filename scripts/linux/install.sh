...
if [[ "$PRUNE" -eq 1 ]]; then
  for name in algorithmic-art auto-skill brand-guidelines browser canvas-design ccxt claude-code-guide claude-cookbooks code-guardrails codeagent coingecko cryptofeed dev do doc-coauthoring docx frontend-design headless-cli hummingbot internal-comms markdown-to-epub mcp-builder myclaude-product-workflow omo pdf polymarket pptx product-requirements prototype-prompt-generator proxychains skill-install slack-gif-creator snapdom sparv test-cases theme-factory web-artifacts-builder webapp-testing xlsx; do
