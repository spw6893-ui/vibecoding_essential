...
本目录保存从 `vibe-coding-cn` 提炼出来的非运行时参考资料。运行时 skill 位于仓库 `skills/`，本目录只作为精选资料库使用，不保存完整上游镜像。

## 目录

- `documents/`：精选方法论、案例、工作流、开发经验和 playbook。
- `prompt/`：提示词素材。
...
- 需要案例参考时读取 `documents/case-studies/`，其中已移除用户明确不需要的交易类案例。
...
- 不保留 `Polymarket`、`ProxyCast` 等用户明确排除或低复用资料。
